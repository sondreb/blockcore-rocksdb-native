...
if [[ $OSINFO == *"MSYS"* || $OSINFO == *"MINGW"* ]]; then
    ...
    (cd rocksdb && {
        ...
        # TODO: Figure out why the detection fails?
        ...
        if [ "$(uname)" == "Darwin" ]; then
            ...
            install_name_tool -change /usr/local/opt/snappy/lib/libsnappy.1.dylib runtimes/osx-x64/native/libsnappy.1.dylib librocksdb.dylib
            install_name_tool -change /usr/local/opt/lz4/lib/liblz4.1.dylib runtimes/osx-x64/native/liblz4.1.dylib librocksdb.dylib
            install_name_tool -change /usr/local/opt/zstd/lib/libzstd.1.dylib runtimes/osx-x64/native/libzstd.1.dylib librocksdb.dylib
